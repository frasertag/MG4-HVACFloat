...
- Themeable expanded bar and hidden handle background colour and opacity
- Configurable hidden `HVAC` handle size, corner shape, background visibility, and text visibility
- Overlay mode selection:
  ...
  - Factory HVAC open state is treated as temporary because SystemUI does not expose a real open/closed callback.
- Autostart modes:
  - `Autostart Full`
  - `Autostart Hidden`
  - `Autostart Off`
...
- Hidden overlay handles still own their touch area; Android does not pass the same single tap through to apps underneath while the overlay is touchable.
...
- `Select Theme`
  ...
  - Uses the custom dark dialog style and applies theme changes to the live overlay immediately.
- `Bar Colour`
  - Adjust the expanded overlay bar and hidden handle background using RGB sliders, a `#RRGGBB` field, and 0-100% opacity.
  - Tapping the colour preview opens an in-app colour picker with swatches and RGB sliders.
- `HVAC Icon Style`
  - Adjust the hidden/factory HVAC handle width, height, corner radius, background visibility, and text visibility.
  - Uses a scrollable dark custom editor so large invisible touch targets can still be configured cleanly.
  - Background and text can both be hidden while leaving the touch target active for easier double-tapping.
- `Overlay Mode`
  - Choose between the normal HVAC Float bar and the factory HVAC overlay launcher handle.
  - Uses the custom dark dialog style.
- `Select Controls`
