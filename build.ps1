...
& $apksigner sign `
  ...
  --out (Join-Path $dist "MG4-HVACFloat-V0.5.3.apk") `
  (Join-Path $build "aligned.apk")

& $apksigner verify --verbose (Join-Path $dist "MG4-HVACFloat-V0.5.3.apk")
Get-Item (Join-Path $dist "MG4-HVACFloat-V0.5.3.apk")
